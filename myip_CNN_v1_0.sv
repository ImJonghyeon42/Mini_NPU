...
(
	...
	// ===== 외부 연결용 포트들 =====
	// Block Design에서 다른 IP들과 연결하기 위한 포트들
	
	// AXI Quad SPI 연결 인터페이스 (선택적 사용)
	output wire [31:0]       o_spi_read_addr,     // SPI 읽기 주소 출력
	output wire              o_spi_read_enable,   // SPI 읽기 활성화
	input wire [31:0]        i_spi_read_data,     // SPI 데이터 입력
	input wire               i_spi_data_valid,    // SPI 데이터 유효
	input wire [10:0]        i_spi_fifo_count,    // SPI FIFO 카운트
	...
	// MicroBlaze controlled pixel interface
	...
	wire [31:0] axi_pixel_reg;     // Pixel data register (REG_PIXEL_DATA)
	...
	// Status register mapping (matches C code bit definitions)
	...
	// AXI Quad SPI 연결 신호들 (필요시 MicroBlaze가 제어)
	assign o_spi_read_addr = 32'h44A00000;  // AXI Quad SPI 기본 주소 (예시)
	assign o_spi_read_enable = cnn_start;   // CNN 시작시 SPI 읽기 활성화
